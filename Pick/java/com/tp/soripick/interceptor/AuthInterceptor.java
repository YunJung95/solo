	...
	MemberService ms;
	 ...
	 @Override //로그인 안했으면 얘가 잡아서 로그인 페이지로 보냄 ((앞으로 이동될 페이지)갈려고 하는 페이지 주소를 가지고 감)
		public boolean preHandle(HttpServletRequest request, 
		 ...
		 //로그인 하지 않은 경우
		 if(session.getAttribute("sMidx") == null){ //sMemberMidx가 없으면 
			// System.out.println("sMidx 빔");
			...
			//쿠키에 저장된 키정보와 같은 키가 DB에 있는지 체크해서 있으면 그 회원정보를 담는다 	
				...
				if (mv != null) {
					 ...
					 request.getSession().setAttribute("sMid", mv.getMid());
					 request.getSession().setAttribute("sMidx", mv.getMidx());
					 request.getSession().setAttribute("sMname", mv.getMname());	
					 ...
					 // DB 테d이블에도 날짜 갱신  //loginCookie는 값을 가지고 있을 수없듬
					...
					// ms.keepLogin(mv.getMidx(), sessionKey, sessionLimit);
	 ...
	 private void saveDest(HttpServletRequest req){ //이동하려고 하는 페이지를 던져주는
		// System.out.println("왔dd다!!! saveDest");

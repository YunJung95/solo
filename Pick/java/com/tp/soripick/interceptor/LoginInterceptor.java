package com.tp.soripick.interceptor;
...
public class LoginInterceptor extends HandlerInterceptorAdapter{ 	// 상속 받음(HandlerInterceptorAdapter)
   ...
   throws Exception {
		...
		Object sMid = modelAndView.getModel().get("sMid");
		Object sMidx = modelAndView.getModel().get("sMidx");
		Object sMname = modelAndView.getModel().get("sMname");
		
		//modelAndView.getModelMap().get 은 객체 꺼낼때
		
		if(sMidx != null){ //컨트롤러에서 담은 모델값 꺼내서 세션에 담기
			//session.setAttribute("memberVo", memberVo); //세션값을 꺼내서 담음
			request.getSession().setAttribute("sMidx", sMidx);
			request.getSession().setAttribute("sMid", sMid);
			request.getSession().setAttribute("sMname", sMname);
	...
	public boolean preHandle(HttpServletRequest request,  HttpServletResponse response, Object handler ) 
		...
		if(session.getAttribute("sMid") != null){ //같은 테이블이 있을까봐 초기화해줌
